...
census_data$Over_50k <- factor(census_data$Over_50k)
head(census_data)
summary(census_data)
```
...
```{r}
p1 <- ggplot(census_data, aes(census_data$Over_50k, fill = factor(census_data$Over_50k))) +geom_bar()
print(p1)
```
```{r}
...
This plot makes it clear that the working class with the greatest proportionof over 50k earners is in the 
```{r}
...
p4 <- ggplot(census_data, aes(x = Occupation, fill = Over_50k )) + 
  geom_bar() + 
  theme( axis.text.x = element_text(angle=45))
...
model <- randomForest(Over_50k~ age + na.omit(training$Race) + na.omit(training$Workclass) + Education_Level,
	data = training,
	importance=TRUE,
	keep.forest=TRUE
)
...
```{r}
mylogit <- glm(Over_50k ~ Sex+ age + Race + Workclass + Education_Level, data = training, family = "binomial")
print(mylogit)
summary(mylogit)
```
...
logitConfusion <- table(actual, predicted)
...
```{r}
predictions <- predict(mylogit, na.omit(testing), type = "response")
mylogitPredictions <- prediction(predictions, list(testing$Over_50K))
...
#AUC
mylogitAUC <- performance(mylogitPredictions, 'auc')
mylogitAUC <- unlist(mylogitAUC@y.values)
mylogitAUC
