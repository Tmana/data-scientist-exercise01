...
We can clearly see that education has a strong relationship with earning over 50k.
...
This plot makes it clear that the working class with the greatest proportion of over 50k earners is in the self-employed-inc group. But we have by far the most entries from the Private catagory, with more over 50k earners than any single other workclass both over and under 50k.
...
```{r}
svm.model <- svm(Over_50k ~ Sex+ age + Race + Workclass + Education_Level, data = training, cost = 50, gamma = 1)
print(svm.model)
summary(svm.model)
```
